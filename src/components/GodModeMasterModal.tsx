...
export const GodModeMasterModal: React.FC<GodModeMasterModalProps> = ({
 ...
 const [currency, setCurrency] = useState(house.settings?.currency ||'?');
 const [lateFeeEnabled, setLateFeeEnabled] = useState(house.settings?.lateFeeEnabled || false);
 const [lateFeeAmount, setLateFeeAmount] = useState((house.settings?.lateFeeAmount || 50).toString());
 const [lateFeeDays, setLateFeeDays] = useState((house.settings?.lateFeeDays || 5).toString());
 const [defaultRentDueDay, setDefaultRentDueDay] = useState((house.settings?.defaultRentDueDay || 1).toString());
 const [maintenanceDueDay, setMaintenanceDueDay] = useState((house.settings?.maintenanceDueDay || 5).toString());
 ...
 setCurrency(house.settings?.currency ||'?');
 setLateFeeEnabled(house.settings?.lateFeeEnabled || false);
 setLateFeeAmount((house.settings?.lateFeeAmount || 50).toString());
 setLateFeeDays((house.settings?.lateFeeDays || 5).toString());
 setDefaultRentDueDay((house.settings?.defaultRentDueDay || 1).toString());
 setMaintenanceDueDay((house.settings?.maintenanceDueDay || 5).toString());
 }, [house]);
 ...
 lateFeeEnabled,
 lateFeeAmount: parseInt(lateFeeAmount, 10) || 0,
 lateFeeDays: parseInt(lateFeeDays, 10) || 0,
 defaultRentDueDay: parseInt(defaultRentDueDay, 10) || 1,
 maintenanceDueDay: parseInt(maintenanceDueDay, 10) || 5,
 }
 });
 playSuccessChime();
 showNotification('Property & Platform Settings updated successfully!');
 };
 ...
 <button
 onClick={onClose}
 className="p-1.5 rounded-xl bg-white hover:bg-white text-white transition-colors cursor-pointer"
 title="Close Editor"
 >
 <X className="w-5 h-5" />
 </button>
 ...
 >
 <Building2 className="w-3.5 h-3.5" /> Property & Platform Settings
 </button>
 ...
 <div>
 <label className="block font-bold text-slate-700 mb-1">Official UPI ID</label>
 <input
 type="text"
 value={upiId}
 ...
 <div>
 <label className="block font-bold text-slate-700 mb-1">UPI Payee Name</label>
 <input
 type="text"
 value={upiName}
 ...
 <div>
 <label className="block font-bold text-slate-700 mb-1">Late Fee Enabled</label>
 <select
 value={lateFeeEnabled ?'true' :'false'}
 onChange={(e) => setLateFeeEnabled(e.target.value ==='true')}
 className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 text-xs"
 >
 ...
 <div>
 <label className="block font-bold text-slate-700 mb-1">Late Fee Amt</label>
 <input
 type="number"
 min="0"
 value={lateFeeAmount}
 ...
 <div>
 <label className="block font-bold text-slate-700 mb-1">Maint. Due Day</label>
 <input
 type="number"
 min="1"
 max="31"
 value={maintenanceDueDay}
 onChange={(e) => setMaintenanceDueDay(e.target.value)}
 className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 text-xs"
 />
 </div>
