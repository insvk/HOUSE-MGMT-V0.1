...
export const AvatarUploadModal: React.FC<AvatarUploadModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div 
        className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-6 py-4 bg-gradient-to-r from-[#405189] to-[#364473] text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-lg bg-white dark:bg-slate-900/10 text-white">
              <Camera className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-bold leading-snug">Update Profile Picture</h2>
              <p className="text-xs text-white/80">{userName} â€¢ {userEmail}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-white dark:bg-slate-900/20 text-white/80 hover:text-white transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="flex flex-col items-center justify-center">
            ...
            <div className="mt-3 text-center">
              <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Live Preview</span>
          ...
          {activeTab === 'presets' && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Choose a professional portrait:</span>
                <button
                  type="button"
                    ...
                    <button
                      ...
                      className={`relative rounded-xl overflow-hidden p-1 border-2 transition-all cursor-pointer group ${
                        isChosen 
                          ...
                          : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:border-slate-600 hover:scale-102'
                      }`}
                    >
                      <img 
                        src={avatar.url} 
          ...
          {activeTab === 'upload' && (
            <div className="space-y-3">
              ...
              <div 
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-[#405189] bg-slate-50 dark:bg-slate-800/50 hover:bg-indigo-50/20 rounded-xl p-8 text-center cursor-pointer transition-all flex flex-col items-center justify-center group"
              >
                <div className="p-3 bg-white dark:bg-slate-900 rounded-full shadow-sm text-slate-500 dark:text-slate-400 group-hover:text-[#405189] group-hover:scale-110 transition-all mb-2.5">
                  <UploadCloud className="w-6 h-6" />
                </div>
                <div className="text-xs font-bold text-slate-700 dark:text-slate-200 group-hover:text-[#405189]">
                  Click to select photo from device
                </div>
                <div className="text-[11px] text-slate-400 mt-1">
                  Supports PNG, JPG, JPEG, WEBP â€¢ Automatically square-cropped & optimized
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'url' && (
            <div className="space-y-3">
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-200">Paste Image Web Address (URL)</label>
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-xs border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:border-[#405189]"
                  />
                </div>
                <button
                  type="button"
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <button
            type="button"
            onClick={handleResetToInitials}
            className="px-3 py-2 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white hover:bg-slate-200 dark:bg-slate-700/50 rounded-lg transition-colors cursor-pointer"
          >
            Reset to Initials
          </button>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:bg-slate-700 rounded-lg transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
