...
export function App() {
  ...
  const toggleTheme = async () => {
    ...
    if (isLoggedIn && currentUser) {
      try {
        const updatedPrefs = { ...(currentUser.preferences || {}), theme: newTheme };
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ml-0 ${
        ...
        <header className="h-[72px] bg-white sticky top-0 z-30 px-4 sm:px-8 flex items-center justify-between border-b border-slate-200 shadow-sm">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            <button
              onClick={toggleTheme}
              className="p-1.5 sm:p-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 cursor-pointer transition-colors"
